...
(function () {
  ...
  function showBootOverlay(message, mode) {
    // Never cover a failed-login state with the prepare/loading splash.
    if (mode !== 'error' && readPendingLoginError()) {
      hideBootOverlay();
      return;
    }
    ensureBootOverlayStyles();
  ...
  function readPendingLoginError() {
    try {
      var params = new URLSearchParams(window.location.search || '');
      var fromQuery = params.get('login_error');
      if (fromQuery) {
        try {
          sessionStorage.setItem('sobha_login_error', fromQuery);
        } catch (e) {}
        return fromQuery;
      }
    } catch (e) {}
    try {
      return sessionStorage.getItem('sobha_login_error') || '';
    } catch (e) {
      return '';
    }
  }

  function clearPendingLoginError() {
    try {
      sessionStorage.removeItem('sobha_login_error');
    } catch (e) {}
  }

  window.sobhaHasPendingLoginError = function () {
    return !!readPendingLoginError();
  };
  ...
  /** Keep the logged-in profile in AUTH even if /users omitted them (e.g. Management). */
  function ensureCurrentUserInAuth() {
    try {
      var saved = sessionStorage.getItem('sobha_user_v5');
      if (!saved) return;
      var u = JSON.parse(saved);
      if (!u || !u.username) return;
      if (!window.AUTH) window.AUTH = {};
      var key = String(u.username).toLowerCase();
      var existing = window.AUTH[key] || window.AUTH[u.username];
      window.AUTH[key] = Object.assign(
        {
          pin: '',
          name: u.name || key,
          role: u.role || 'viewer',
          scope_type: u.scope_type || 'all',
          scope_value: u.scope_value || 'ALL',
          email: u.email || '',
        },
        existing || {},
        {
          name: u.name || (existing && existing.name) || key,
          role: u.role || (existing && existing.role) || 'viewer',
          scope_type: u.scope_type || (existing && existing.scope_type) || 'all',
          scope_value: u.scope_value || (existing && existing.scope_value) || 'ALL',
          email: u.email || (existing && existing.email) || '',
        }
      );
    } catch (e) {}
  }
  ...
  function fetchWithTimeout(promise, ms, label) {
    return new Promise(function (resolve, reject) {
      ...
      var timer = window.setTimeout(function () {
        ...
        reject(new Error((label || 'Request') + ' timed out. Please refresh and try again.'));
      }, ms);
      promise.then(
        function (value) {
          if (done) return;
  ...
  window.sobhaBootFromServer = function () {
    ...
    // Join an in-flight boot instead of bailing (that left the splash stuck).
    ...
    bootPromise = fetchWithTimeout(fetchWithRetry(10, 1000), BOOT_TIMEOUT_MS, 'Loading SBTR')
      .then(function (fresh) {
        ...
        try {
          ...
          console.error('[dashboard-data] init after fetch failed:', err);
          // Still reveal UI — session is valid and data is loaded.
        }
        startDatasetPoll();
        return true;
      })
      ...
      .finally(function () {
        bootInFlight = false;
        bootPromise = null;
        // Always clear the splash after a successful data load path.
        if (window.__sobhaApiInitDone) {
          window.setTimeout(hideBootOverlay, 100);
        }
      });
  ...
  function showLoginErrorFromQuery() {
    try {
      var msg = readPendingLoginError();
      if (!msg) return false;

      // Persist before clearing the query string — unpack runs later and must
      // still know login failed (otherwise prepare splash covers the form).
      try {
        sessionStorage.setItem('sobha_login_error', msg);
      } catch (e) {}
      try {
      ...
      paintError();
      window.setTimeout(paintError, 50);
      window.setTimeout(paintError, 200);
      window.setTimeout(paintError, 800);
      window.setTimeout(function () {
        paintError();
  ...
  window.sobhaPinLogin = function sobhaPinLogin() {
    ...
    pinLoginBusy = true;
    clearPendingLoginError();
    document.documentElement.classList.remove('sobha-login-error');
    if (err) {
  ...
  window.sobhaInstallDashboardHooks = function () {
    wrapSobhaInit();
    patchDoLogin();
    // Dashboard-only mobile CSS (no-op on desktop widths; never touches /admin).
    try {
      if (!document.getElementById('sobhaMobileCss') && window.location.pathname.indexOf('/admin') !== 0) {
        var link = document.createElement('link');
        link.id = 'sobhaMobileCss';
        link.rel = 'stylesheet';
        link.href = '/js/dashboard-mobile.css?v=1';
        (document.head || document.documentElement).appendChild(link);
      }
    } catch (e) {}
    if (showLoginErrorFromQuery()) {
      return;
    }
    if (!token()) {
      hideBootOverlay();
  ...
  installDoLoginTrap();
  installPinLoginCaptureHandlers();
  var hasLoginError = !!readPendingLoginError();
  ...
  // Never re-cover after a failed PIN login redirect (?login_error=...).
  if (token()) {
    showBootOverlay('Fetching the latest sales performance report');
