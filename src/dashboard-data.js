...
(function () {
  ...
  function showLoginErrorFromQuery() {
    try {
      ...
      // Failed PIN login — never keep the prepare splash up.
      hideBootOverlay();
      try {
      ...
      paintError();
      // Bundle may remount login a moment later — re-apply.
  ...
  try {
    hasLoginError = !!(new URLSearchParams(window.location.search || '').get('login_error'));
  } catch (e) {}
