...
def _dashboard_login_html_error(message: str, status_code: int = 401) -> HTMLResponse:
    """
    Failed PIN login must never bounce through the dashboard bundler splash.

    Return a self-contained error page so the user sees the message immediately
    instead of "Preparing your workspace" while the SPA unpacks.
    """
    msg_js = json.dumps(message)
    msg_html = html.escape(message or "Invalid username or password")
    return HTMLResponse(
        f"""<!DOCTYPE html>
<html><head>
<meta charset="utf-8">
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  body {{
    ...
    background: #f8f6f1; font-family: Calibri, "Trebuchet MS", sans-serif; color: #1a1814;
  }}
  .card {{
    background: #fff; border: 1px solid #e8e3d7; border-radius: 4px;
    padding: 40px 36px; width: 100%; max-width: 420px;
    box-shadow: 0 12px 32px rgba(20,16,8,0.08); position: relative; text-align: center;
  }}
  .card::before {{
    content: ""; position: absolute; top: 0; left: 0; right: 0; height: 3px;
    background: linear-gradient(90deg, #8a6938, #b08a4e, #8a6938);
  }}
  .brand {{
    font-family: Georgia, "Times New Roman", serif; font-size: 28px; font-weight: 700;
    letter-spacing: 0.28em; color: #b08a4e; margin-bottom: 20px;
  }}
  h1 {{ font-size: 18px; margin-bottom: 14px; }}
  .err {{
    background: #f1cfcf; border: 1px solid #e3a8a8; border-radius: 3px;
    color: #7a2929; font-size: 13px; padding: 12px 14px; margin-bottom: 24px;
  }}
  a.btn {{
    display: inline-block; width: 100%; padding: 14px; text-decoration: none;
    background: #0d0c0a; color: #b08a4e; border-radius: 3px;
...
sessionStorage.setItem("sobha_login_error", {msg_js});
</script>
</body></html>""",
        status_code=status_code,
    )
